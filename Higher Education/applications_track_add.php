...
if (isActionAccessible($guid, $connection2, '/modules/Higher Education/applications_track_add.php') == false) {
    ...
    if (studentEnrolment($_SESSION[$guid]['gibbonPersonID'], $connection2) == false) {
        ...
        if ($result->rowCount() != 1) {
            ...
            $data = array();
            $sql = "SELECT higherEducationInstitutionID as value, concat(name, ' (', country, ')') as name FROM higherEducationInstitution WHERE active='Y' ORDER BY name";
            $row = $form->addRow();
                $row->addLabel('higherEducationInstitutionID', __('Institution'));
                $row->addSelect('higherEducationInstitutionID')->fromQuery($pdo, $sql, $data)->placeholder()->required();
