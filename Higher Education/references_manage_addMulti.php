    ...
    if ($role == false) {
        ...
        if ($role != 'Coordinator') {
            ...
            $form->addHiddenValue('address', $_SESSION[$guid]['address']);
                ...
                $sql = "SELECT gibbonPerson.gibbonPersonID as value, concat(gibbonRollGroup.nameShort, ' - ', gibbonPerson.surname,', ',gibbonPerson.firstName) as name FROM gibbonPerson JOIN gibbonStudentEnrolment ON (gibbonPerson.gibbonPersonID=gibbonStudentEnrolment.gibbonPersonID) JOIN gibbonRollGroup ON  (gibbonStudentEnrolment.gibbonRollGroupID=gibbonRollGroup.gibbonRollGroupID) JOIN higherEducationStudent ON (higherEducationStudent.gibbonPersonID=gibbonPerson.gibbonPersonID) WHERE status='FULL' AND (dateStart IS NULL OR dateStart<='".date('Y-m-d')."') AND (dateEnd IS NULL  OR dateEnd>='".date('Y-m-d')."') AND gibbonRollGroup.gibbonSchoolYearID=:gibbonSchoolYearID ORDER BY nameShort, surname, preferredName";
