...
if (isActionAccessible($guid, $connection2, "/modules/Higher Education/student_manage_delete.php")==FALSE) {
	...
	if ($higherEducationStudentID=="") {
		...
		if ($result->rowCount()!=1) {
			...
			$row=$result->fetch() ;
			?>
			<form method="post" action="<?php print $_SESSION[$guid]["absoluteURL"] . "/modules/" . $_SESSION[$guid]["module"] . "/student_manage_deleteProcess.php?higherEducationStudentID=$higherEducationStudentID" ?>">
				<table class='smallIntBorder' cellspacing='0' style="width: 100%">	
					<tr>
						<td> 
							<b>Are you sure you want to delete "<?php print formatName("", $row["preferredName"], $row["surname"], "Student", true, true) ?>" from the Higher Education programme?</b><br/>
					...
					<tr>
						<td> 
							<input name="higherEducationStudentID" id="higherEducationStudentID" value="<?php print $higherEducationStudentID ?>" type="hidden">
